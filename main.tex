...
\cite{Xarray_HoyerHamman2017,ZarrPython_Zenodo2025,HLAVO_GitHub}. Volba formátu
...
Povrchová část systému vychází z fyzikálního popisu proudění vody ve variably
saturovaném prostředí. Základní rovnicí je Richardsova rovnice, kterou lze v
...
závislá na nasycení \cite{Farthing_Ogden_2017}. Materiálové vztahy jsou v
systému zadávány pomocí van Genuchtenova typu parametrizace, například pro
retenční křivku
\[
  ...
  m=1-\frac{1}{n},
\]
pro nenasycený stav. Tento popis umožňuje převádět meteorologické buzení a
vlastnosti půdy na časový průběh vlhkosti a na tok vody do hlubší části
systému.

V implementaci HLAVO je tato úloha řešena nad simulátorem ParFlow
\cite{ParFlow_MaxwellCondonKollet2015}. Pro jednotlivá stanoviště je uvažován
...
dalšími meteorologickými veličinami \cite{Priestley_Assessment_1972,CLM5_Lawrence2019}.
...
je povrchový model zabalen do Unscented Kalmánova filtru. Ten po každém kroku
...
půdě \cite{Evensen2003,Camporese2009}. Praktické zkušenosti z vývoje ukázaly,
...
časových intervalech pomocí evoluční optimalizace s průběžnou křížovou
...
tlakových výšek v jednotlivých buňkách a malou sadou materiálových parametrů,
které určují retenční a vodivostní vlastnosti půdy. V aktuálně používané
integrované konfiguraci je vedle tlakových výšek standardně asimilována
především efektivní nasycená hydraulická vodivost $K_s$; implementace je však
připravena i pro rozšíření o další van Genuchtenovy parametry nebo o kalibrační
koeficienty navázané na jednotlivé hloubky. Měřicí vektor je tvořen vlhkostmi v
hloubkách, kde jsou na daném stanovišti umístěna čidla.

Pro další výklad stačí velmi stručné abstraktní schéma UKF 
\includegraphics[width=\textwidth]{fig/Basic_concept_of_Kalman_filtering.pdf}
\cite{Wikipedia_UKF}.
\begin{enumerate}
  \setlength{\itemsep}{0pt}
  \setlength{\parskip}{0pt}
  \item Z aktuálního odhadu $\hat{\vc x}_{k-1}$ a kovariance $\tn P_{k-1}$ se
  vytvoří sigma body
  \[
    \vc\chi^{(0)}_{k-1}=\hat{\vc x}_{k-1},\qquad
    \vc\chi^{(i)}_{k-1}=\hat{\vc x}_{k-1}\pm
    \left(\sqrt{(n+\lambda)\tn P_{k-1}}\right)_i .
  \]
  \item Sigma body se propagují nelineárním modelem
  \[
    \vc\chi^{(i)-}_{k}=f\!\left(\vc\chi^{(i)}_{k-1},\vc u_k\right),
  \]
  a z nich se složí predikovaný stav a jeho kovariance
  \[
    \hat{\vc x}^{-}_{k}=\sum_i W_i^{(m)}\vc\chi^{(i)-}_{k},\qquad
    \tn P^{-}_{k}=\sum_i W_i^{(c)}
    \left(\vc\chi^{(i)-}_{k}-\hat{\vc x}^{-}_{k}\right)
    \left(\vc\chi^{(i)-}_{k}-\hat{\vc x}^{-}_{k}\right)^\top+\tn Q.
  \]
  \item Predikované sigma body se převedou měřicí funkcí
  \[
    \vc\gamma^{(i)}_{k}=h\!\left(\vc\chi^{(i)-}_{k}\right),
  \]
  čímž dostaneme predikované měření, jeho kovarianci a křížovou kovarianci
  \[
    \hat{\vc y}_{k}=\sum_i W_i^{(m)}\vc\gamma^{(i)}_{k},\qquad
    \tn S_k=\sum_i W_i^{(c)}
    \left(\vc\gamma^{(i)}_{k}-\hat{\vc y}_{k}\right)
    \left(\vc\gamma^{(i)}_{k}-\hat{\vc y}_{k}\right)^\top+\tn R,
  \]
  \[
    \tn P^{xy}_k=\sum_i W_i^{(c)}
    \left(\vc\chi^{(i)-}_{k}-\hat{\vc x}^{-}_{k}\right)
    \left(\vc\gamma^{(i)}_{k}-\hat{\vc y}_{k}\right)^\top.
  \]
  \item Korekce se provede pomocí Kalmánova zisku
  \[
    \tn K_k=\tn P^{xy}_k \tn S_k^{-1},
    \qquad
    \hat{\vc x}_{k}=\hat{\vc x}^{-}_{k}+\tn K_k\left(\vc y_k-\hat{\vc y}_{k}\right),
    \qquad
    \tn P_k=\tn P^{-}_{k}-\tn K_k \tn S_k \tn K_k^\top.
  \]
\end{enumerate}

V našem případě je nejdůležitější, jak jsou obsazeny abstraktní prvky
$f$, $\tn Q$, $\tn R$ a v menší míře i počáteční kovariance $\tn P_0$. Funkce
$f$ není zjednodušený regresní krok, ale přímo dopředná simulace 1D modelu
ParFlow se zadaným meteorologickým buzením a se spodní okrajovou podmínkou
přebíranou z hlubokého modelu. Měřicí funkce $h$ převádí tento stav na vlhkosti
v hloubkách čidel pomocí interpolace a materiálových vztahů.
...
parametry. Právě volba $\tn Q$ určuje, nakolik filtr připouští, že se model od
reality odchýlí i bez opory v novém měření.
...
vstupů pro MODFLOW~6 \cite{MODFLOW6_Framework_Hughes2017,HLAVO_GitHub}.
...
Dashboard využívá stejné datové sklady jako výpočetní část systému a zobrazuje
profilová měření, vrty i meteorologické vstupy nad jednotným mapovým podkladem.
Tím odpadá potřeba vytvářet zvláštní exporty jen pro kontrolu dat. Uživatel má
možnost rychle porovnat prostorové rozmístění stanic, časové řady měření i
vybrané simulační veličiny. Tato vrstva je důležitá nejen pro interpretaci
výsledků, ale i pro provozní kontrolu kvality vstupních dat a pro budoucí
nasazení systému mimo hlavní testovací lokalitu.
